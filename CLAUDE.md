# networking: instructions for AI assistants
...
The Bespok3d networking co-repo publishes mesh-VPN plugins and the kernel base they build on, shipping
Klipper/Moonraker config and patches, binaries, and shell wrappers only (no Python shipped to the
printer):

- `tun-module`: carries a cross-built `tun.ko` and loads it, so the printer has a working
  `/dev/net/tun`. The base a mesh VPN needs.
- `zerotier`: runs ZeroTier's `zerotier-one` as a managed service and joins a user-supplied network id.
  Requires `tun-module`.
- `tailscale`: runs Tailscale's `tailscaled` as a managed service and joins a user-supplied tailnet with
  an auth key. Requires `tun-module`.
...
4. **Run the gate and make it green:** `bash scripts/check.sh`. The gate needs the `lib_bespok3d`
   ...
   (workflow-pinning, the em-dash guard, and shellcheck) plus each wrapper's own regression tests: shell
   tests for `ts-run` and `zt-run` against fake tailscaled/zerotier binaries, and a Python test of the
   Moonraker trusted-range merge (see `scripts/check.sh` for the exact targets).
