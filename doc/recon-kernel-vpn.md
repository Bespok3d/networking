...
| `Bespok3d-desktop/scripts/pack-plugins.sh` | Packer / gate, not a builder | n/a | central `ensure_baked`/`check_baked_deps`/`find_baker` (lines ~249-320): walks up to the owning repo's `scripts/bake-deps.sh`, refuses to emit a `.b3` for a plugin declaring Python deps without baked artifacts; a Python-deps-only gate today |
...
> section's referenced docs: ADR-0014, `Bespok3d_history/doc/package-format.md`, `daemon/core/intent.py`). Build
