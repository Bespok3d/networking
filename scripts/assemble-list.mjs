...
const LIST_NAME = 'Bespok3d Networking'
...
function registerProvider(providers, atom, provided) {
...
function providerByService(atoms) {
  const providers = {}
  atoms.forEach((atom) => {
    ;(atom.provides ?? []).forEach((provided) => registerProvider(providers, atom, provided))
  })
  return providers
}
