...
# <plugin-id>/ dir (manifest.json + files/ + optional doc/), matching the u1-extras co-repo
# convention. A plugin whose payload is a build output (tun-module's cross-built .ko, zerotier's
# downloaded binary) ships a build.sh that stages it into files/ BEFORE this script runs; this
# script only zips what is already on disk. Always-repack (no lockfile/auto-bump); bump a plugin's
...
file_mode() { if [ -x "$1" ]; then echo "755"; else echo "644"; fi; }
